...
> Prisma 7 使用 `@prisma/adapter-pg` + `pg` 驱动，无 Rust query engine；Docker 镜像需 Node ≥ 20.19，构建前执行 `prisma generate`（见 `apps/server/package.json` 的 `build` 脚本）。
...
> 本项目为 **Prisma ORM 7**：`DATABASE_URL` 在 `apps/server/prisma.config.ts`；构建/开发机需 **Node ≥ 20.19**。详见 [技术实现文档 · Prisma 7](技术实现文档.md#231-prisma-orm-7)。

### 6.1 流程

```bash
# 0. 确认 Node 版本（避免 ERR_REQUIRE_ESM）
node -v   # ≥ v20.19.0

# 1. 本地修改 prisma/schema.prisma 后
pnpm --filter @translator/server prisma:migrate --name <change-desc>
# 注意：Prisma 7 的 migrate dev 不会自动 generate / seed

# 2. 若 schema 变更影响 Client，生成并一并提交
pnpm --filter @translator/server prisma:generate
git add apps/server/prisma/migrations apps/server/src/generated/prisma

# 3. （仅开发库）可选种子
pnpm --filter @translator/server prisma:seed

# 4. 提交
git commit -m "feat(server): <数据库变更说明>"

# 5. CI/CD 在部署前执行（生产禁止 seed）
